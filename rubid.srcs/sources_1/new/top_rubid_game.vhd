...
ENTITY top_rubid_game IS
    PORT (
        ...
        CPU_RESETN : IN STD_LOGIC; --active low, set to init state 
        ...
        -- SW(5 downto 0)  : The 6 faces (D, B, L, U, R, F), 
...
ARCHITECTURE Structural OF top_rubid_game IS
    ...
    COMPONENT master_game_controller
        PORT (
            Clk : IN STD_LOGIC;
            Reset : IN STD_LOGIC;           -- reset game state 
            BTN_Start : IN STD_LOGIC;       
            SW_Mode : IN STD_LOGIC;         
            Time_Is_Zero : IN STD_LOGIC;    --time out flag from timer 
            Is_Solved : IN STD_LOGIC;       --solved flag from cube
            Scramble_Done: in STD_LOGIC;    -- connect to scrambler to trigger 
            ...
            LED_Win : OUT STD_LOGIC;        -- logically equivalent to Is_solve ทำไปทำไม
            LED_Lose : OUT STD_LOGIC);           
    END COMPONENT;

    -- 1 human press = exactly 1 digital pulse.
    ...
    COMPONENT move_controller
        PORT (
            ...
            RESET, F, R, U, L, B, D : IN STD_LOGIC; -- selects which face to turn, and reset cube face
    ...
    COMPONENT mux_3bit_2to1
        PORT (
            Sel : in STD_LOGIC; --connect ot master controller 
    ...
    COMPONENT sexy_move_detector
        PORT (
            ...
            reset : IN STD_LOGIC;                    -- reset the sequence 
    ...
    SIGNAL sys_reset : STD_LOGIC; -- was used as game mode reset 

    -- debounced Button Wires
    SIGNAL clean_btn_exec : STD_LOGIC; 
    SIGNAL clean_btn_start : STD_LOGIC; 
    SIGNAL clean_btn_reset_cube : STD_LOGIC; 
    ...
    SIGNAL move_controller_S_Out : STD_LOGIC_VECTOR(2 DOWNTO 0); -- move controller to mux to cube
    --data
    SIGNAL cube_memory_out : STD_LOGIC_VECTOR(71 DOWNTO 0); --connect cube to video controller 
...
BEGIN
    ...
    sys_reset <= NOT CPU_RESETN; -- pin นี้active low แต่อยากใช้active high 
    ...
    -- reset cube face to move controller 
    -- คือกดแุ่มอย่างเเดียวไม่ได้ต้องกด execute ให้ด้วย 
   ...
   U_Debounce_ResetCube : button_debouncer PORT MAP(
    ...
    U_SevenSeg : seven_segment_controller PORT MAP(
        Clk     => CLK100MHZ,
        Reset   => sys_reset,
        Time_In => current_time_wire, -- Reads the time from the timer
    ...
    U_Sexy_Detector : sexy_move_detector PORT MAP(
        ...
        SW_Direction => SW(6), 
        reset => auto_boot_reset, -- Resets on either secure reset or system boot
